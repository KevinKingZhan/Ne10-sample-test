...
void FFTTester::test() {
	ne10_fft_cpx_float32_t src[SAMPLES] = { { 0.0000, 0.0 }, { 0.0227, 0.0 }, { 0.0479, 0.0 }, { 0.0167, 0.0 }, { 0.2529, 0.0 }, { 0.5894, 0.0 }, { 0.1660, 0.0 }, { 0.8999, 0.0 },
			{ 0.7818, 0.0 }, { 0.4206, 0.0 }, { 0.1260, 0.0 }, { 0.4038, 0.0 }, { 0.1730, 0.0 }, { 0.2540, 0.0 }, { 0.0593, 0.0 }, { 0.0293, 0.0 } };
	ne10_fft_cpx_float32_t dst[SAMPLES];
	ne10_fft_cfg_float32_t cfg;

	if (ne10_init() != NE10_OK) {
		LOGI("Failed to initialise Ne10.\n");
	}
	cfg = ne10_fft_alloc_c2c_float32(SAMPLES);
	ne10_fft_c2c_1d_float32_neon(dst, src, cfg, 0);
	for (int i = 0; i < SAMPLES; i++) {
		LOGI("IN[%2d]: %10.4f %10.4f\t", i, src[i].r, src[i].i);
		LOGI("OUT[%2d]: %10.4f %10.4f\n", i, dst[i].r, dst[i].i);
	}
